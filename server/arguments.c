    ...
    switch (key){
        ...
        case ARGP_KEY_ARG:
            if(state->arg_num == 0 && !args->port_set){
                ...
                argp_usage(state); //muchos argumentos
            }
            break;
        case ARGP_KEY_END:
            if(args->port == 21 && geteuid() != 0){
                argp_error(state, "Permiso denegado: el puerto 21 requiere privilegios de root");
            }
            if(args->port != 21 && (args->port < IPPORT_RESERVED || args->port > IPPORT_USERRESERVED)){
                argp_error(state, "Puerto invalido: %d. Permitido: 21 (como root), or %d %d", args->port, IPPORT_RESERVED, IPPORT_USERRESERVED);
            }
            if(!is_valid_local_ip(args->address)){
                argp_error(state, "IP invalida: %s no es una interface local", args->address);
            }
            break;
        default:
...
int parse_arguments(int argc, char **argv, struct arguments *args){
    //Initialize defaults:
    args->port = FTP_PORT; //sera 'PORT_DEFAULT' ???
    args->port_set = 0;
